        ...
        switch(v.getId()){
            case R.id.confirm_button_configure_post:
                ...
                if(TextUtils.isEmpty(post_title)){
                    ...
                    toCreatePost.putExtra("location_description",location_description.toString());
            ...
            switch(what){
                case Config.REQUEST_GET_LOCATION_DESCRIPTION:
                    if(response.getHeaders().getResponseCode()==200){
                        location_description =response.get();
                        try {
                            int permission=location_description.getInt("permission");
                            switch(permission){
