...
def login():
    ...
    if request.method == 'POST':
        ...
        findUser=session.query(User).filter(User.user_id==user_id).first()
        if findUser==None:
...
@app.route('/alter_basic_information',methods=['POST','GET'])
...
@app.route('/alter_password',methods=['POST','GET'])
def alter_password():
    session=get_session()
    result=dict()
    if request.method=="POST":
...
@app.route('/get_location_description',methods=['POST','GET'])
...
@app.route('/get_post_detail',methods=['POST','GET'])
def get_post_detail():
    session=get_session()
    if request.method=='POST':
...
@app.route('/get_post_picture',methods=['POST','GET'])
...
@app.route('/like_post_or_not',methods=['POST','GET'])
def like_post_or_not():
    ...
    if request.method=='POST':
        ...
        if has_liked=='1':
            ...
            session.add(newLike)
            # remove old condemn
            oldCondemn=session.query(Condemn).filter(Condemn.post_id==post_id and Condemn.user_id==user_id).first()
            if oldCondemn is not None:
                session.delete(oldCondemn)
                post.condemn_number-=1
            session.commit()
            result = {}
            result['permission'] = LIKE_POST_SUCCESS
        else:
            like=session.query(Like).filter(Like.post_id==post_id and Like.user_id==user_id).first()
            session.delete(like)
            post.like_number=post.like_number-1
            session.commit()
            result={}
...
@app.route('/condemn_post_or_not',methods=['POST','GET'])
def condemn_post_or_not():
    ...
    if request.method == 'POST':
        ...
        if has_condemned=='1':
            ...
            session.add(newCondemn)
            # remove old like
            oldLike=session.query(Like).filter(Like.post_id==post_id and Like.user_id==user_id).first()
            ...
            condemn=session.query(Condemn).filter(Condemn.post_id==post_id and Condemn.user_id==user_id).first()
            post.condemn_number=post.condemn_number-1
            session.delete(condemn)
            session.commit()
            result={}
